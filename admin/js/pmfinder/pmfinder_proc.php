...
function pmf_image_process($file, $path)
{
	...
	query(
		"insert into _vfs (real_name, real_path, virtual_name, virtual_path) values ('$file_uid.{$pp['extension']}', '$dest', '$file', '$parent_vp/$file')"
	);
...
function pmf_folder_create($path, $name)
{
	...
	if (!@mkdir($dir_path, 0777, true)) return array(
		'err' => "На удалось создать папку:<br>$name",
		'real_name' => $real_name
	);
	query(
		"insert into _vfs (real_name, real_path, virtual_name, virtual_path) values ('$dir_uid', '$dir_path', '$name', '$parent_vp/$name')"
	);
