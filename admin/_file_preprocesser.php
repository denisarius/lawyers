		...
		switch ($pp['extension'])
		{
			...
			case 'css':
				if ($is_processing)
				{
					...
					if (isset($_gallery_thumbnail_size)) $file = str_replace(
					...
					if (isset($_gallery_thumbnail_size)) $file = str_replace(
						'{@gallery_item_width@}',
						$_gallery_thumbnail_size + 10,
						$file
					);
					if (isset($_gallery_thumbnail_size)) $file = str_replace(
						'{@gallery_item_height@}',
						$_gallery_thumbnail_size + 100,
						$file
					);

					if (isset($_cms_objects_image_sx)) $file = str_replace(
						'{@object_image_sx@}',
						$_cms_objects_image_sx,
						$file
					);
					if (isset($_cms_objects_image_sy)) $file = str_replace(
						'{@object_image_sy@}',
						$_cms_objects_image_sy,
						$file
					);

					if (isset($_cms_person_photo_size_sx)) $file = str_replace(
						'{@staff_photo_size_sx@}',
						$_cms_person_photo_size_sx,
						$file
					);
					if (isset($_cms_person_photo_size_sy)) $file = str_replace(
						'{@staff_photo_size_sy@}',
						$_cms_person_photo_size_sy,
						$file
					);
					if (isset($_cms_person_photo_quality)) $file = str_replace(
						'{@staff_photo_photo_quality@}',
						$_cms_person_photo_quality,
						$file
					);

					if (isset($_actions_image_sx)) $file = str_replace(
						'{@actions_image_sx@}',
						$_actions_image_sx,
						$file
					);
					if (isset($_actions_image_sy)) $file = str_replace(
						'{@actions_image_sy@}',
						$_actions_image_sy,
						$file
					);

					if (isset($_cms_banners_image_sx)) $file = str_replace(
						'{@banners_image_sx@}',
						$_cms_banners_image_sx,
						$file
					);
					if (isset($_cms_banners_image_sy)) $file = str_replace(
						'{@banners_image_sy@}',
						$_cms_banners_image_sy,
						$file
					);

					if (isset($_cms_attachments_file_types)) $file = str_replace(
						'{@attachments_file_types@}',
						$_cms_attachments_file_types,
						$file
					);
				}
				break;
