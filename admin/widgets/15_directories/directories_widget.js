...
//------------------------------------------------------------------------------
// Directory delete
function dirs_dir_delete()
{
	dir_id=$("#dirs_current_dir").val();
	if (dir_id==-1)
	{
		show_message_warning('Удаление справочника', 'Для удаления необходимо выюрать справочник');
		return;
	}
	mask_screen(function(){
		confirm_box('Удаление справочника', 'Вы действительно хотите удалить этот справочник? При удалении будут удалены все элементы данного справочника.', function(){
			res=execQuery(directories_widget_ajax_manager_url, {section : "dirsDirDelete", dir_id: dir_id});
			if (res!='')
				show_message_warning('Удаление данных', res);
			else
			{
				$("#dirs_current_dir").val(-1);
				$("#dirs_dir_content").html('');
				$("#dirs_current_dir option[value='"+dir_id+"']").remove();
			}
		});
	});
	clear_mask();
...
//------------------------------------------------------------------------------
function dirs_dir_online_value_add_save(func)
{
	val=$("#dirs_value_content").val().trim();
	if (val=='')
	{
	...
	dir_id=$("#dirs_dir_edit_id").val();
	menu_id=$("#dirs_link_to_menu_id").val();
//	mask_screen(function(){
		res=execQuery(directories_widget_ajax_manager_url, {section : "dirsDirValueSave", dir_id: dir_id, val_id: -1, val: val, menu_id: menu_id});
		if (res!='')
		{
			res=eval('(' + res + ')');
			if (res.error!='')
