...
switch ($section)
{
	...
	case 'menusDeleteMenu':
		...
		{
			$res = query(
				"select id from $_cms_texts_table where menu_item in (select id from $_cms_menus_items_table where menu='$id')"
			);
			if (mysql_num_rows($res))
				echo 'texts';
			else
	...
	case 'menusAddMenuItem':
		...
		else
		{
			...
			query(
				"insert into $_cms_menus_items_table (name, url, parent, menu, sort, visible) values ('$name', '', '$parent', '$menu', '$max', 1)"
			);
			$status = 'ok';
	...
	case 'menusMenuItemGetData':
		...
		if (isset($id) && $id != '')
		{
			...
			echo serialize_data(
				'id|name|parent|subtree|url|tag|visible|buttons',
				$r['id'],
				$r['name'],
				$r['parent'],
				$subtree,
	...
	case 'menusMenuItemSaveData':
		...
		if (isset($id) && $id != '' && isset($name) && name != '')
			query(
				"update $_cms_menus_items_table set name='$name', url='$url', tag='$tag', visible='$visible' where id='$id'"
			);
