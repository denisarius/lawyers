...
switch ($section)
{
	...
	case 'textsEditTextSave':
		...
		if (isset($id) && $id != '' && isset($menu) && $menu != '' && isset($signature) &&
			$signature != '' &&
			isset($title) &&
			$title !=
		...
		)
		{
			...
			{
				query(
					"insert into $_cms_texts_table (menu_item, date, signature, title, keywords, descr, content, visible) values ('$menu', CURDATE(), '$signature', '$title', '$kw', '$descr', '$content', 1)"
				);
				echo 'Текст записан.';
			}
			else
			{
				query(
					"update $_cms_texts_table set title='$title', keywords='$kw', descr='$descr', content='$content' where id='$id'"
				);
