...
function texts_get_texts_list($menu, $page, $strip_slashes = true)
{
	...
	$res = query(
		"select SQL_CALC_FOUND_ROWS id, date, title, signature, substring(content, 1, 300) as content from $_cms_texts_table where menu_item='$menu' order by id desc limit $start, $_cms_texts_admin_list_page_length"
	);
	$start = $page*$_cms_texts_admin_list_page_length;
	...
	if ($totalPages > 0 and $page > $totalPages - 1)
	{
		...
		$res = query(
			"select id, date, title, signature, substring(content, 1, 300) as content from $_cms_texts_table where menu_item='$menu' order by id desc limit $start, $_cms_texts_admin_list_page_length"
		);
	}
	$html = <<<stop
