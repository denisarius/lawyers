...
var directories_widget_ajax_manager_url='{@widgets_url@}/{@widget_path_directories@}/directories_widget_dm.php';
// -----------------------------------------------------------------------------
...
function typed_objects_object_edit_save()
{
	...
	res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsEditCanBeSave", id: obj_id, name: obj_name, menu_item: menu_item, type:obj_type});
	if (res!='')
	{
...
function typed_objects_object_move_process(menu, copy_mode)
{
    ...
    mask_screen(function(){
		...
		res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsMoveObject", id: id, menu: menu, copy: copy_mode});
		typed_objects_show_objects_list(false);
//		if (res!='') alert(res);
	});
	clear_mask();
...
//------------------------------------------------------------------------------
var typed_objects_attachment_prop_id=0;
function typed_objects_prop_edit_attachment_edit(id)
{
	typed_objects_attachment_prop_id=id;
...
{
	id=typed_objects_attachment_prop_id;
	$("#propf_"+id).val(file);
	$("#propn_"+id).val(name);
...
// -----------------------------------------------------------------------------
var typed_objects_dir_prop_id=0;
function typed_objects_prop_edit_dir_add(dir_id, prop_id)
{
	typed_objects_dir_prop_id=prop_id;
	admin_info_show('Добавить значение', "<img src='/admin/images/"+loading_icon+"'>", 200);
	res=execQuery(directories_widget_ajax_manager_url, {section : "dirDirAddGetHTML", dir_id: dir_id, func: "typed_objects_prop_edit_dir_add_save"});
	admin_info_change('', res, 600);
}
//------------------------------------------------------------------------------
function typed_objects_prop_edit_dir_add_save(dir_id, dir_value_id, dir_value_text)
{
	res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsGetDirSelectItemsHTML", dir_id: dir_id, value_id: dir_value_id});
	admin_info_close();
//	alert(res);
	if (!typed_objects_dir_prop_id) return;
//	$("#prop_"+typed_objects_dir_prop_id).empty();
	$("#prop_"+typed_objects_dir_prop_id).html(res);
}
//------------------------------------------------------------------------------
var typed_objects_menu_prop_id=0;
function typed_objects_property_menu_item_select(type, id)
{
	typed_objects_menu_prop_id=id;
	admin_info_show('Выберите подраздел', '<div style="text-align: center"><img src="images/'+loading_icon+'"></div>');
	res=execQuery(common_widget_ajax_manager_url, {section : "commonGetLinkMenuHTML", func: 'typed_objects_menu_selected'});
	admin_info_change('', res, null);
}
//------------------------------------------------------------------------------
function typed_objects_menu_selected(id, menu, menu_item)
{
	res=execQuery(typed_objects_widget_ajax_manager_url, {section : "typed_objectsPropEditGetMenuPropText", menu_id: id});
	$("#prop_"+typed_objects_menu_prop_id).val(id);
	$("#propt_"+typed_objects_menu_prop_id).html(res);
	admin_info_close();
}
//------------------------------------------------------------------------------
