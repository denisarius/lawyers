...
{
	global $_o, $_cms_objects_types, $_cms_objects_table, $_cms_objects_admin_list_page_length;
...
stop;
	...
	if (isset($object_description['admin_sort']) && $object_description['admin_sort']!='') {
		$s_field=$object_description['admin_sort'];
		$q=<<<stop
select SQL_CALC_FOUND_ROWS obj.*, d0.value as sort_field
from $_cms_objects_table as obj
left join {$_o['cms_objects_details']} as d0
	on obj.id=d0.node and d0.typeId='$s_field'
where
	obj.menu_item='$menu_item' and
	obj.type='$obj_type'
order by sort, sort_field desc, obj.id desc
limit $start, $_cms_objects_admin_list_page_length
stop;
		$sort_btn='';
	}
	else {
...
from $_cms_objects_table
where
	menu_item='$menu_item' and
	type='$obj_type'
order by sort, id desc
limit $start, $_cms_objects_admin_list_page_length
stop;
		$sort_btn=<<<stop
<input type="button" value="Сортировка объектов" id="typed_objects_object_sort" onClick="typed_objects_object_sort()" style="float:right;">
stop;
	}
	$res=query($q);
...
function typed_objects_get_edit_object_html($id, $type)
{
	...
	if ($id==-1) {
		...
		$obj_img_src="$_admin_root_url/images/no_image_256.gif";
		if ($obj_img!='') {
			$image_file="$_base_site_objects_images_path/$obj_img";
			if (file_exists($image_file)) {
				copy_image_to_temp($image_file);
				$obj_img_src="$_base_site_objects_images_url/$obj_img";
			}
		}
...
// копируем изображения связанные с полями в temp
		...
		foreach($object_description['details'] as $obj_prop)
			if($obj_prop['type']=='st') {
				...
				while($r=mysql_fetch_assoc($res)) {
					if ($r['image']!='') {
						$image_file="$_base_site_structured_text_images_path/{$r['image']}";
						if (file_exists($image_file))
							copy_image_to_temp($image_file);
					}
				}
...
stop;
	...
	$html.=<<<stop
<table width='99%'>
<tr><td valign='top' width='120' class='typed_objects_edit_prop_need_title'>Название</td>
...
stop;
	        ...
	        case 'menu':
		        $v=get_data('value', $_cms_objects_details, "node='$id' and typeId='{$obj_prop['id']}'");
				if ($v!==false)
					$text=typed_objects_get_menu_item_value_text($v);
				else
					$text='выберите раздел';
		        $html.=<<<stop
<input type="hidden" id="prop_{$obj_prop['id']}" name="{$obj_prop['id']}" value="$v">
<div class="typed_objects_property_menu_values" id="propt_{$obj_prop['id']}" onClick="typed_objects_property_menu_item_select($type, '{$obj_prop['id']}')">
	$text
</div>
stop;
		        break;
...
// -----------------------------------------------------------------------------
/**
 * Function generate the text string for menu item property
 * @param $menu_item_id
 */
function typed_objects_get_menu_item_value_text($menu_item_id)
{
	global $_o;

	$menu=get_data_array('*', $_o['cms_menus_items_table'], "id='$menu_item_id'");
	$text=$menu['name'];
	while($menu['parent']) {
		$menu=get_data_array('*', $_o['cms_menus_items_table'], "id='{$menu['parent']}'");
		$text="{$menu['name']} > $text";
	}
	$menu_name=get_data('name', $_o['cms_menus_table'], "id='{$menu['menu']}'");
	return "$menu_name :: $text";
}
// -----------------------------------------------------------------------------
...
stop;
	for($i=0; $i<$desc['columns_count']; $i++) {
		if (isset($col_width[$i])) $width=$col_width[$i];
		else $width=$desc['cellwidth'];
		$html.=<<<stop
<div class="hdr" style="width: {$width}px;">{$desc['columns'][$i]}</div>
stop;
	}
	$html.='</div>';
	...
	for($i=0; $i<count($cells); $i+=$desc['columns_count']) {
		$html.=typed_objects_table_edit_get_row_html($i/$desc['columns_count'], $cells, $i, $desc, $col_width, true);
	}
	$max_row=count($cells)/$desc['columns_count'];
	$html.=typed_objects_table_edit_get_row_html($max_row, array_fill(0, $desc['columns_count'], ''), 0, $desc, $col_width, false);
	$html.=<<<stop
	</div>
...
stop;
	...
	for($i=$pos; $i<$pos+$desc['columns_count']; $i++)
	{
		...
		if (isset($cellwidth[$i])) $width=$cellwidth[$i];
		else $width=$desc['cellwidth'];
		$html.=<<<stop
<input class="typed_object_table_edit_node_cell" type="text" value="$cell" style="width: {$width}px;" onKeyUp="typed_object_table_edit_row_key_control($row_id)"/>
stop;
	}
...
// -----------------------------------------------------------------------------
// Check ability of objects data save
function typed_objects_edit_can_be_save($id, $name, $menu_item, $type)
{
	global $html_charset, $_cms_objects_table;

	$object_description=typed_objects_get_object_description($type);
	if (!isset($object_description['can_have_same_name']) || !$object_description['can_have_same_name']) {
...
/**
 * Object copying or moving procedure
 * @param integer $id ID of object to copy
 * @param integer $menu ID of menu_item (table $_cms_menus_items_table) to copy
 * @param integer $copy_mode mode of operation (0-move; 1-copy)
 */
	function typed_objects_object_move($id, $menu, $copy_mode)
{
	...
	global $_cms_text_parts, $_base_site_structured_text_images_path, $_base_site_objects_images_path;
	...
	if(!$copy_mode)
		...
		// Copy the image of object if it exists
		$old_image=get_data('image', $_cms_objects_table, "id='$id'");
		if ($old_image!='') {
			$dest=create_unique_file_name($_base_site_objects_images_path, $old_image);
			$pp=pathinfo($dest);
			@copy("$_base_site_objects_images_path/$old_image", $dest);
			query("update $_cms_objects_table set image='{$pp['basename']}' where id='$new_id'");
		}
		...
		// Copy the linked gallery images and data if gallery is linked
...
function typed_objects_object_delete($id)
{
	...
	global $_base_site_objects_images_path;
	...
	// Удаляем картинку объекта
	$image=get_data('image', $_cms_objects_table, "id='$id'");
	if ($image!='') {
		@unlink("$_base_site_objects_images_path/$image");
	}

	// Удаляем данные объекта из таблиц
	query("delete from $_cms_objects_details where node='$id'");
	query("delete from $_cms_objects_table where id='$id'");

	// Удаляем связанные галереи
	if (isset($_cms_gallery_table) && $_cms_gallery_table!='') {
		$res=query("select * from $_cms_gallery_table where menu_item='$id' and link_type=1");
		while($r=mysql_fetch_assoc($res)) {
...
// -----------------------------------------------------------------------------
// Generate the HTML code for select object
function typed_objects_get_dir_select_items($dir_id, $val_id)
{
	global $_cms_directories_data;

	$res=query("select * from $_cms_directories_data where dir='$dir_id'");
	$html='';
	while($r=mysql_fetch_assoc($res))
	{
		$sl='';
		if ($r['id']==$val_id) $sl="selected=' selected'";
		$html.="<option value='{$r['id']}'$sl>{$r['content']}</option>";
	}
	return $html;
}
