...
stop;
			$ot = get_data_array(
				'type, count(*) as cnt',
				$_cms_objects_table,
				"menu_item='$menu_item' group by type order by cnt desc limit 1"
			);
			$ot = $ot['type'];
...
stop;
	...
	$res = query(
		"select SQL_CALC_FOUND_ROWS * from $_cms_objects_table where menu_item='$menu_item' and type='$obj_type' order by sort, id desc limit $start, $_cms_objects_admin_list_page_length"
	);
	$total = get_data('FOUND_ROWS()');
	$html .= get_admin_pager(
		$total,
		$page,
		$_cms_objects_admin_list_page_length,
		'typed_objects_show_objects_list_page'
	);
...
function typed_objects_get_object_propertis_edit_html($id, $type)
{
	...
	foreach ($object_description['details'] as $obj_prop)
	{
		...
		if (isset($obj_prop['input_width']) &&
			$obj_prop['input_width'] != ''
		) $input_width = "{$obj_prop['input_width']}px";
...
stop;
			...
			case 'dm':
				$resV = query(
					"select value from $_cms_objects_details where node='$id' and typeId='{$obj_prop['id']}'"
				);
				$val = array();
				$hval = '';
...
stop;
	...
	for ($i = 0; $i < count($cells); $i += $desc['columns_count'])
		$html .= typed_objects_table_edit_get_row_html(
			$i/$desc['columns_count'],
			$cells,
			$i,
			$desc,
			$desc['cellwidth'],
	...
	$html .= typed_objects_table_edit_get_row_html(
		$max_row,
		array_fill(0, $desc['columns_count'], ''),
		0,
		$desc,
		$desc['cellwidth'],
		false
...
stop;
	...
	for ($i = 0; $i < count($fragments); $i += 4)
		$html .= typed_objects_structured_text_edit_get_fragment_html(
			$fragments[$i],
			$fragments[$i + 1],
			$fragments[$i + 2],
			$fragments[$i + 3],
...
// удаляем записи в таблице $_cms_text_parts и связанные изображения
	...
	else
	{
		...
		query(
			"insert into $_cms_objects_table (menu_item, type, name, note, date, visible, image, gallery) values ('$menu_item', '$obj_type', '$name', '$note', CURDATE(), 0, '$img', '$gallery')"
		);
		$id = mysql_insert_id();
	}
			...
			switch ($type)
			{
				case 'tb':
					...
					for ($i = 0; $i < count($fr); $i += $desc['columns_count'])
					{
						...
						query(
							"insert into $_cms_objects_details (node, typeId, type, value) values ('$id', '$k', '$type', '$str')"
						);
					}
					break;
				case 'st':
					query(
						"insert into $_cms_objects_details (node, typeId, type, value) values ('$id', '$k', '$type', '')"
					);
					...
					for ($i = 0; $i < count($fr); $i += 4)
					{
						...
						query(
							"insert into $_cms_text_parts (type, node, date, title, image, content, sort, visible) values (1, '$node_id', CURDATE(), '$title', '{$fr[
																																								 $i +
																																								 2]}', '$content', '{$fr[$i]}', 1)"
						);
					}
				...
				case 'dm': // множественный выбор из справочника
					...
					foreach ($ve as $v)
						query(
							"insert into $_cms_objects_details (node, typeId, type, value) values ('$id', '$k', '$type', '$v')"
						);
					break;
				case 'date': // string of date
					...
					$date_v = "$y-$m-$d";
					query(
						"insert into $_cms_objects_details (node, typeId, type, value) values ('$id', '$k', '$type', '$date_v')"
					);
					break;
				case 'html':
				...
				case 'file':
					...
					if ($attachment_id !== false)
						query(
							"insert into $_cms_objects_details (node, typeId, type, value) values ('$id', '$k', '$type', '$attachment_id')"
						);
					break;
				default:
					query(
...
function typed_objects_object_move($id, $menu, $copy_mode)
{
	...
	else
	{
		...
		query(
			"insert into $_cms_objects_table (menu_item, $fields_list) select '$menu' as menu_item, $fields_list from $_cms_objects_table where id='$id'"
		);
		$new_id = mysql_insert_id();
		// Creating the new records in the object details table
		$fields_list = mysql_get_fields_list($_cms_objects_details, 'id|node');
		query(
			"insert into $_cms_objects_details (node, $fields_list) select $new_id as node, $fields_list from $_cms_objects_details where node='$id'"
		);
		if (isset($_cms_gallery_table) && $_cms_gallery_table != '')
		{
			...
			while ($r = mysql_fetch_assoc($res))
			{
				...
				query(
					"insert into $_cms_gallery_table (menu_item, file, title, comment, sort, visible, link_type) values ('$new_id', '{$pp['basename']}', '{$r['title']}', '{$r['comment']}', '{$r['sort']}', '{$r['visible']}', '{$r['link_type']}')"
				);
			}
			mysql_free_result($res);
			...
			query(
				"insert into $_cms_gallery_data_table (menu_item, $fields_list) select $new_id as menu_item, $fields_list from $_cms_gallery_data_table where menu_item='$id'"
			);
		}
...
//		echo "new ID[$new_id] ".mysql_num_rows($resT)."\n";
		...
		while ($rt = mysql_fetch_assoc($resT))
		{
			...
			query(
				"insert into $_cms_text_parts (node, $fields_list) select {$rt['id']} as node, $fields_list from $_cms_text_parts where node='$old_node' and type=1"
			);
