...
function pmTextContentTransform($content)
{
	...
	while ($pos < strlen($content))
	{
		...
		if ($in_pm_tag)
		{
			if (substr($content, $pos, 6) == '&quot;')
			{
				$content = substr($content, 0, $pos).'"'.substr($content, $pos + 6);
				$c_char = '"';
			}
			if (substr($content, $pos, 5) == '&#39;')
			{
				$content = substr($content, 0, $pos).'\''.substr($content, $pos + 5);
				$c_char = '\'';
			}
			$tag .= $c_char;
			if (!$in_quotes)
...
function pmImageResize($im, $max_x, $max_y, $img_file, $ext, $jpg_quality, $max = 0)
{
	...
	switch ($ext)
	{
		case 'jpg':
		case 'jpeg':
			imagejpeg($imd, $img_file, $jpg_quality);
			break;
		case 'png':
